...
def _ensure_mold_design_column(conn: sqlite3.Connection) -> None:
...
def ingest(payload: dict[str, Any], db_path: str | Path, evidence_root: str | Path) -> dict[str, Any]:
    ...
    with _connect(db_path) as conn:
        _ensure_mold_design_column(conn)
        ...
        existing = conn.execute(
            "SELECT id FROM muestras WHERE mesa_id=? AND codigo_muestra=?",
            (mesa_id, code),
        ).fetchone()
        if existing:
            ...
            conn.execute(
                ...
                (date_value, event_ref, event_origin, declared, sample_type, color,
                 texture, logo, mold_design, mold_fingerprint, foto_ref, notes_value, sample_id),
            )
            duplicate = True
        else:
            cur = conn.execute(
