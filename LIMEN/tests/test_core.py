...
from LIMEN.core.contracts import Checkpoint, Event, EventRecord, ExplicitAction, Proposal, Snapshot, TimestampError
...
class EventAndReplayTests(unittest.TestCase):
    ...
    def test_serialized_event_and_checkpoint_contracts_reject_type_coercion(self):
        ...
        invalid_schema = dict(event_wire, schema_version="1")
        with self.assertRaises(ValueError):
            Event.from_dict(invalid_schema)

        invalid_payload = dict(event_wire, payload=[("value", 1)])
        with self.assertRaises(ValueError):
            Event.from_dict(invalid_payload)
        ...
        checkpoint_wire = checkpoint.to_dict()
        with self.assertRaises(ValueError):
